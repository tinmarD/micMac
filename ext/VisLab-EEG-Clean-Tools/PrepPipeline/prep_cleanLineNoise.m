function [signal, lineNoiseOut] = prep_cleanLineNoise(signal, lineNoiseIn, titre)
...
%
%
% Modified by Martin Deudon for integration in micMac (Nov. 2016)
...
try parfor_progress(length(chans)); catch; end;
tic;
parfor ch = chans
    data(ch, :) = removeLinesMovingWindow(squeeze(data(ch, :)), lineNoiseOut); 
    try parfor_progress(); catch; end;
end
toc;
